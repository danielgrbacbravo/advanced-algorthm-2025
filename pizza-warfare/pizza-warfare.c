...
Computer *scan_computers(uint64_t number_of_computers) {
  ...
  for (uint64_t i = 0; i < number_of_computers; i++) {
    if (scanf("%lu %lu", &computers[i].burst_size,
              &computers[i].burst_frequency) != 2) {
      free(computers);
      return NULL;
    }
  }
...
__int128 count_requests_up_to(__int128 time, __int128 target_request_count,
                              int number_of_computers, Computer *computers) {
...
__int128 find_minimum_time_for_requests(uint64_t required_request_count,
                                        uint64_t number_of_computers,
                                        Computer *computers) {
  // We want to find the minimum time such that count_requests_up_to(time, ...)
  // >= required_request_count
  __int128 min_time = 0;

  // For each computer, calculate the time it would take to produce all required
  // requests alone, then take the minimum of these times as a reasonable upper
  // bound.
  __int128 max_time = 0;
  for (uint64_t i = 0; i < number_of_computers; i++) {
    if (computers[i].burst_size > 0) {
      __int128 bursts_needed =
          (required_request_count + computers[i].burst_size - 1) /
          computers[i].burst_size;
  ...
  while (count_requests_up_to(max_time, required_request_count,
                              number_of_computers,
                              computers) < required_request_count) {
    max_time *= 2;
  }
  ...
  while (min_time < max_time) {
    ...
    __int128 current_request_count = count_requests_up_to(
        mid_time, required_request_count, number_of_computers, computers);
...
int main(void) {
  ...
  uint64_t target_requests = read128();
  ...
  __int128 min_required_time = find_minimum_time_for_requests(target_requests, number_of_computers,
                                 computers);

  printf("%lu\n", (uint64_t)min_required_time);
